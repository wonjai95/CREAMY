...
//	    java.sql.Date date = java.sql.Date.valueOf(res_date);
//	    System.out.println("예약날짜 : " + date);
		
		if (user_id != null) {
//			vo.setUser_id(user_id);												// 회원아이디
//			vo.setRes_state(req.getParameter("selectType"));					// 예약상태(방문?예약중?예약완료?)
////			vo.setRes_date(date);											// 예약날짜
//			vo.setRes_hour(Integer.parseInt(req.getParameter("selectTime")));	// 예약시간
//			vo.setEmployee_code(req.getParameter("selectManager"));				// 예약담당자
//			vo.setRes_cnt(Integer.parseInt(req.getParameter("GuestCount")));	// 예약인원수
//			vo.setRes_indiv_request(req.getParameter("res_indiv_request"));		// 특별요청(추가요청)
//			vo.setProduct_code(req.getParameter("ReserveProduct"));				// 예약한 상품서비스
//			vo.setRes_room(req.getParameter("selectRoom"));						// 예약호실
